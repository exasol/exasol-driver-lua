...
local function deep_equals(object1, object2)
    ...
    if type1 ~= 'table' and type2 ~= 'table' then return object1 == object2 end
    for key1, value1 in pairs(object1) do
        local value2 = object2[key1]
        if value2 == nil or not deep_equals(value1, value2) then return false end
    end
    for key2, value2 in pairs(object2) do
        local value1 = object1[key2]
        if value1 == nil or not deep_equals(value1, value2) then return false end
    end
    return true
end
...
local function is_json(state, arguments)
    ...
    return function(actual)
        ...
        if not equal then log.warn("Expected %s but got %s", cjson.encode(expected), cjson.encode(actual)) end
        return equal
    end
end
