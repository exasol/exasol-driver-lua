...
local function wsupgrade(wsconn)
    ...
    local uhead = {}
    for k, v in pairs(wsconn.options.upgrade_headers or {}) do table.insert(uhead, k .. ": " .. v) end
    ...
    if nb == nil then return false, "Failed to send upgrade request: " .. tostring(err) end
...
local function connect(ip, port)
    ...
    if not sock then return nil, "Can't get socket for connection" end
    sock:settimeout(15)
    local r, e = sock:connect(ip, port)
    if r then return sock end
...
function wsopen(url, handler, options)
    ...
    if not proto then error("Invalid protocol/address for WebSocket open in " .. url) end
...
local function send_frame(wsconn, opcode, fin, s)
    ...
    local t = {}
    local b = (fin and 0x80 or 0) | opcode
    ...
    if mask then
        ...
        for k = 1, #s do table.insert(t, string.char(string.byte(s, k) ~ mb[((k - 1) % 4) + 1])) end
...
function wssend(wsconn, opcode, s)
    ...
    if #s < 65535 then
        ...
        repeat
            ...
            if not send_frame(wsconn, opcode, fin, chunk) then return false, "send error" end
...
local function wshandlefragment(fin, op, data, wsconn)
    ...
    if fin then
        ...
        if maxn < #data then
            D("wshandlefragment() buffer overflow, have %1, incoming %2, max %3; message truncated.", #wsconn.msg,
              #data, wsconn.options.max_payload_size)
        end
        ...
        local ok, err = pcall(wsconn.msghandler, wsconn, wsconn.msgop, wsconn.msg,
                              unpack(wsconn.options.handler_args or {}))
        if not ok then L("wsandlefragment() message handler threw error:", err) end
        wsconn.msg = nil
    else
        -- No FIN
        if (wsconn.msg or "") == "" then
            ...
            if op ~= 0 then
                return pcall(wsconn.msghandler, wsconn, false, "ws continuation error",
                             unpack(wsconn.options.handler_args or {}))
            end
            local maxn = math.max(0, wsconn.options.max_payload_size - #wsconn.msg)
            if maxn < #data then
                L("wshandlefragment() buffer overflow, have %1, incoming %2, max %3; message truncated", #wsconn.msg,
                  #data, wsconn.options.max_payload_size)
            end
            if maxn > 0 then wsconn.msg = wsconn.msg .. data:sub(1, maxn) end
        end
...
local function unmask(fragt, maskt)
    ...
    for _, d in ipairs(fragt or {}) do
        for l = 1, #d do
            ...
            table.insert(r, string.char(string.byte(d, l) ~ maskt[k]))
        end
    end
